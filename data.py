...
class DataSeq:
    ...
    # Add the MAX_IM_SIZE attribute here
    MAX_IM_SIZE = 500
    ...
    def __init__(self, Length, time_interval=1,
        ...
        if sound and pygame is not None:
            ...
            # Initialize the sound_list attribute
            self.sound_list = np.array([])

            # Initialize the sound_mixer attribute
            self.sound_mixer = []
    ...
    def GetSoundArray(self):
        pygame.mixer.init(SF, -16, 2, 2048)  # Set channels to 2 for stereo audio

        # Reshape self.sound_list to have two channels (stereo audio)
        self.sound_list = self.sound_list.reshape(-1, 2)
